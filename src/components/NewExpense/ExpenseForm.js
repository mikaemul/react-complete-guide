...
/* ASSIGNMENT 4 
  ...
  if (toggle === "on") {
    shownInformation = (
      <form onSubmit={submitHandler}>
        ...
        <div className="new-expense__actions">
          <button type="button" onClick={closeExpenseFormHandler}>
            Cancel
          </button>
          <button type="submit">Add Expense</button>
        </div>
    ...
    shownInformation = (
      <div>
        <button type="button" onClick={openExpenseFormHandler}>
          Add New Expense
        </button>
      </div>
    );
  ...
  return <div>{shownInformation}</div>;
}; */
